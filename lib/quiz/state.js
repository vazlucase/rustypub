...
export function publicQuestion(config, attemptId, index, deadlineAt, optionOrderSlot = null) {
  const question = getQuestionByIndex(index);
  if (!question) return null;
  const options = deterministicOptionOrder(config.optionSecret, config.campaignId, attemptId, question, optionOrderSlot)
    .map(option => ({
...
export function publicAttemptState(config, attemptId, attempt, ticketCode = null, ticketStatus = 'issued') {
  ...
  if (status === 'active') {
    return {
      ...base,
      question: publicQuestion(config, attemptId, index, Number(attempt.deadlineAt), attempt.optionOrderSlot)
    };
  }
  const result = {
